...
bool Database::proc_33(::hyde::rt::Vec<Tup_i32> vec35, ::hyde::rt::Vec<Tup_i32_i32> vec45, ::hyde::rt::Vec<Tup_i32> vec50, ::hyde::rt::Vec<Tup_i32_i32> vec54) {
  ...
  vec50.Clear();
  vec54.Clear();
  flow_145(std::move(vec39), std::move(vec42), std::move(vec53), std::move(vec62));
  return false;
}
...
bool Database::flow_145(::hyde::rt::Vec<Tup_i32> vec39, ::hyde::rt::Vec<Tup_i32> vec42, ::hyde::rt::Vec<Tup_i32> vec53, ::hyde::rt::Vec<Tup_i32> vec62) {
      ...
      for (auto [v87] : vec43) {
        if (const uint32_t j86_0 = table_15.Find({v87}); j86_0 != ::hyde::rt::kNoRow) {
          ...
          for (uint32_t j86_1 = idx_88.First({v87}); j86_1 != ::hyde::rt::kNoRow; j86_1 = idx_88.Next(j86_1)) {
            ...
            if (v87 == v89 && v87 == v90) {
              if (const auto ins6 = table_15.TryChangeAbsentToPresent({v91}); ins6.changed) {
  ...
  for (bool reenter58 = true; reenter58; ) {
    for (bool changed58 = true; changed58; changed58 = !(true && vec59.Empty() && vec62.Empty() && vec64.Empty())) {
      vec60.Clear();
      vec59.SortAndUnique();
      vec59.Swap(vec60);
      for (auto [v94] : vec60) {
        ...
        if (const uint32_t j104_0 = out_12.Find({v105}); j104_0 != ::hyde::rt::kNoRow) {
          ...
          for (uint32_t j104_1 = idx_106.First({v105}); j104_1 != ::hyde::rt::kNoRow; j104_1 = idx_106.Next(j104_1)) {
            ...
            if (v105 == v107 && v105 == v108) {
              if (const auto ins10 = table_29.TryChangeAbsentToPresent({v109, true}); ins10.changed) {
                if (ins10.added_row) {
        ...
        if (const uint32_t j110_0 = __5.Find({v111}); j110_0 != ::hyde::rt::kNoRow) {
          ...
          for (uint32_t j110_1 = idx_112.First({v111}); j110_1 != ::hyde::rt::kNoRow; j110_1 = idx_112.Next(j110_1)) {
            ...
            if (v111 == v113 && v111 == v114) {
              if (const auto ins11 = out_12.TryChangeAbsentToPresent({v115}); ins11.changed) {
    ...
    vec65.Clear();
    vec61.Clear();
    reenter58 = !(true && vec59.Empty() && vec62.Empty() && vec64.Empty());
  }
  return true;
}
