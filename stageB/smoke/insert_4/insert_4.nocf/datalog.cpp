        ...
        for (uint32_t s34 = 0; s34 < table_26.NumRows(); ++s34) {
          ...
          if (find_36(v35)) {
            if (table_5.TryChangePresentToUnknown({true, v35})) {
              if (off_vals_9.TryChangePresentToUnknown({v35})) {
              }
            }
          }
        }
    ...
    if (const auto ins2 = table_26.TryChangeAbsentToPresent({v42}); ins2.changed) {
      if (!find_43(true)) {
        if (const auto ins3 = table_5.TryChangeAbsentOrUnknownToPresent({true, v42}); ins3.changed) {
          ...
          if (const auto ins4 = off_vals_9.TryChangeAbsentOrUnknownToPresent({v42}); ins4.changed) {
          }
        }
      }
      if (const auto ins5 = table_22.TryChangeAbsentToPresent({v42, true}); ins5.changed) {
        if (ins5.added_row) {
          idx_48.Add({true}, ins5.id);
        }
        vec39.Add({true});
      }
    }
  ...
  switch (off_vals_9.State({v64})) {
    ...
    case ::hyde::rt::TupleState::kUnknown: {
      if (off_vals_9.TryChangeUnknownToAbsent({v64})) {
        if (find_65(v64)) {
          if (const auto ins6 = off_vals_9.TryChangeAbsentToPresent({v64}); ins6.changed) {
            return true;
          }
        } else {
          return false;
  ...
  switch (table_5.State({v77, v78})) {
    ...
    case ::hyde::rt::TupleState::kUnknown: {
      if (table_5.TryChangeUnknownToAbsent({v77, v78})) {
        if (find_80(v77, v78)) {
          if (const auto ins7 = table_5.TryChangeAbsentToPresent({v77, v78}); ins7.changed) {
            if (ins7.added_row) {
              idx_72.Add({v78}, ins7.id);
            }
            return true;
          }
        } else {
  ...
  for (auto [v47] : vec39) {
    if (const uint32_t j46_0 = __12.Find({v47}); j46_0 != ::hyde::rt::kNoRow) {
      ...
      for (uint32_t j46_1 = idx_48.First({v47}); j46_1 != ::hyde::rt::kNoRow; j46_1 = idx_48.Next(j46_1)) {
        ...
        if (v47 == v49 && v47 == v50) {
          if (const auto ins8 = on_vals_19.TryChangeAbsentToPresent({v51}); ins8.changed) {
          }
        }
      }
    }
