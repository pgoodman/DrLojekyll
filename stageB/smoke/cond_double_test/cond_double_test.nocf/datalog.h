...
// Key of `idx_33` over `table_18`.
struct Key33 {
  bool c1;
  uint64_t Hash(void) const noexcept {
    return ::hyde::rt::HashRow(c1);
  }
  bool operator==(const Key33 &) const noexcept = default;
};
...
class Database {
 public:
  ...
  // Message `feed_r/1`.
  bool feed_r_1(::hyde::rt::Vec<Tup_i32> vec38);

  // Message `set_c/1`.
  bool set_c_1(::hyde::rt::Vec<Tup_i32> vec42);
 ...
 private:
  bool init_4();
  bool proc_22(::hyde::rt::Vec<Tup_i32> vec24, ::hyde::rt::Vec<Tup_i32> vec28);
  bool flow_48(::hyde::rt::Vec<Tup_b> vec27);
