...
bool Database::proc_33(::hyde::rt::Vec<Tup_u64_u64> vec36, ::hyde::rt::Vec<Tup_u64> vec41, ::hyde::rt::Vec<Tup_i32> vec45) {
  ...
  for (auto [v38, v39] : vec36) {
    if (std::make_tuple(v38) < std::make_tuple(v39)) {
      out_13.TryChangeAbsentToPresent({v38});
  ...
  for (auto [v43] : vec41) {
    if (const auto ins1 = table_26.TryChangeAbsentToPresent({v43}); ins1.changed) {
      vec44.Add({v43});
    }
  }
  for (auto [v47] : vec45) {
    if (const auto ins2 = table_9.TryChangeAbsentToPresent({true, v47}); ins2.changed) {
      if (const auto ins3 = __6.TryChangeAbsentToPresent({true}); ins3.changed) {
        vec35.Add({true});
      }
    }
  }
...
bool Database::flow_76(::hyde::rt::Vec<Tup_b> vec35, ::hyde::rt::Vec<Tup_u64> vec40, ::hyde::rt::Vec<Tup_u64> vec44) {
  if ((g34 += 1) == 1) {
    if (const auto ins4 = table_29.TryChangeAbsentToPresent({1, true}); ins4.changed) {
      if (ins4.added_row) {
        idx_50.Add({true}, ins4.id);
      }
      vec35.Add({true});
    }
  }
  ...
  for (auto [v55, v56] : stage54) {
    out2_16.TryChangeAbsentToPresent({v55, v56});
  }
  vec40.Clear();
  vec44.Clear();
  vec35.SortAndUnique();
  for (auto [v49] : vec35) {
    if (const uint32_t j48_0 = __6.Find({v49}); j48_0 != ::hyde::rt::kNoRow) {
      ...
      for (uint32_t j48_1 = idx_50.First({v49}); j48_1 != ::hyde::rt::kNoRow; j48_1 = idx_50.Next(j48_1)) {
        ...
        if (v49 == v51 && v49 == v52) {
          proof_20.TryChangeAbsentToPresent({v53});
        }
      }
    }
  }
