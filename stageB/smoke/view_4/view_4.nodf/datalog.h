...
class Database {
 public:
  ...
  // Message `in/1`.
  bool in_1(::hyde::rt::Vec<Tup_i32> vec39);

  // Message `log_in/1`.
  bool log_in_1(::hyde::rt::Vec<Tup_i32> vec43);

  // Query `q_chain/1` (f).
  struct q_chain_f_cursor {
    Database &db;
    uint32_t pos;
    bool next(int32_t &X);
  };
  q_chain_f_cursor q_chain_f();

  // Query `q_gated/1` (f).
  struct q_gated_f_cursor {
    Database &db;
    uint32_t pos;
    bool next(int32_t &X);
  };
  q_gated_f_cursor q_gated_f();

 private:
  bool init_4();
  bool proc_23(::hyde::rt::Vec<Tup_i32> vec25, ::hyde::rt::Vec<Tup_i32> vec29);
  bool flow_49(::hyde::rt::Vec<Tup_b> vec28);
  ...
  ::hyde::rt::Table<Row13> q_chain_13;
  ::hyde::rt::Table<Row16> q_gated_16;
