...
bool Database::proc_23(::hyde::rt::Vec<Tup_i32> vec25, ::hyde::rt::Vec<Tup_i32> vec29) {
  ...
  for (auto [v27] : vec25) {
    if (!(v27 == 0)) {
      if (const auto ins0 = q_chain_13.TryChangeAbsentToPresent({v27}); ins0.changed) {
      }
    }
    if (const auto ins1 = table_19.TryChangeAbsentToPresent({v27, true}); ins1.changed) {
      if (ins1.added_row) {
        idx_34.Add({true}, ins1.id);
      }
      vec28.Add({true});
    }
  }
  for (auto [v31] : vec29) {
    if (const auto ins2 = table_9.TryChangeAbsentToPresent({true, v31}); ins2.changed) {
  ...
  for (auto [v33] : vec28) {
    if (const uint32_t j32_0 = __6.Find({v33}); j32_0 != ::hyde::rt::kNoRow) {
      ...
      for (uint32_t j32_1 = idx_34.First({v33}); j32_1 != ::hyde::rt::kNoRow; j32_1 = idx_34.Next(j32_1)) {
        ...
        if (v33 == v35 && v33 == v36) {
          if (!(v37 == 0)) {
            if (const auto ins4 = q_gated_16.TryChangeAbsentToPresent({v37}); ins4.changed) {
            }
          }
        }
      }
