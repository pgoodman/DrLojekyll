...
// Key of `idx_47` over `table_19`.
struct Key47 {
  bool c1;
  uint64_t Hash(void) const noexcept {
    return ::hyde::rt::HashRow(c1);
  }
  bool operator==(const Key47 &) const noexcept = default;
};
...
class Database {
 public:
  ...
  // Message `cond_src/1`.
  bool cond_src_1(::hyde::rt::Vec<Tup_i32> vec52);

  // Message `in/1`.
  bool in_1(::hyde::rt::Vec<Tup_i32> vec56);
 ...
 private:
  bool init_4();
  bool proc_23(::hyde::rt::Vec<Tup_i32> vec31, ::hyde::rt::Vec<Tup_i32> vec35);
  bool flow_62(::hyde::rt::Vec<Tup_i32> vec26, ::hyde::rt::Vec<Tup_b> vec28);
