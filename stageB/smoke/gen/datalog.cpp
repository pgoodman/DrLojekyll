...
Database::Database(::hyde::rt::Allocator allocator_, DatabaseLog &log_, DatabaseFunctors &functors_)
    : allocator(allocator_),
      ...
      __5(allocator_),
      table_8(allocator_),
      user_is_logged_in_12(allocator_),
      table_15(allocator_),
      table_18(allocator_),
      table_21(allocator_),
      idx_41(allocator_) {
  ...
  for (auto [v40] : vec30) {
    if (const uint32_t j39_0 = __5.Find({v40}); j39_0 != ::hyde::rt::kNoRow) {
      ...
      for (uint32_t j39_1 = idx_41.First({v40}); j39_1 != ::hyde::rt::kNoRow; j39_1 = idx_41.Next(j39_1)) {
        ...
        if (v40 == v42 && v40 == v43) {
          if (const auto ins5 = user_is_logged_in_12.TryChangeAbsentToPresent({v44}); ins5.changed) {
          }
        }
      }
    }
...
bool Database::user_is_logged_in_b(int32_t UserId) {
  return user_is_logged_in_12.State({UserId}) == ::hyde::rt::TupleState::kPresent;
}
