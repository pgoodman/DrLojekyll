...
// Key of `idx_54` over `table_35`.
struct Key54 {
  bool c1;
  uint64_t Hash(void) const noexcept {
    return ::hyde::rt::HashRow(c1);
  }
  bool operator==(const Key54 &) const noexcept = default;
};
...
class Database {
 public:
  ...
  // Message `something/1`.
  bool something_1(::hyde::rt::Vec<Tup_i32> vec64);

  // Message `a/1`.
  bool a_1(::hyde::rt::Vec<Tup_i32> vec69);

  // Message `b/1`.
  bool b_1(::hyde::rt::Vec<Tup_i32> vec74);
 ...
 private:
  bool init_4();
  bool proc_39(::hyde::rt::Vec<Tup_i32> vec43, ::hyde::rt::Vec<Tup_i32> vec46, ::hyde::rt::Vec<Tup_i32> vec49);
  bool flow_82(::hyde::rt::Vec<Tup_b> vec41, ::hyde::rt::Vec<Tup_b> vec42);
