...
bool Database::proc_19(::hyde::rt::Vec<Tup_i32> vec21) {
  ...
  for (auto [v23] : vec21) {
    if (const auto ins0 = table_15.TryChangeAbsentToPresent({v23, true}); ins0.changed) {
      if (ins0.added_row) {
        idx_27.Add({true}, ins0.id);
      }
      vec24.Add({true});
    }
    if (const auto ins1 = table_8.TryChangeAbsentToPresent({true, v23}); ins1.changed) {
  ...
  for (auto [v26] : vec24) {
    if (const uint32_t j25_0 = __5.Find({v26}); j25_0 != ::hyde::rt::kNoRow) {
      ...
      for (uint32_t j25_1 = idx_27.First({v26}); j25_1 != ::hyde::rt::kNoRow; j25_1 = idx_27.Next(j25_1)) {
        ...
        if (v26 == v28 && v26 == v29) {
          if (const auto ins3 = out_12.TryChangeAbsentToPresent({v30}); ins3.changed) {
          }
        }
      }
    }
