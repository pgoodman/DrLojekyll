...
int main() {
  ...
  print("t1", collect(db.t1_out_f()));
  db.m1_1(nodes({1}));
  print("t1", collect(db.t1_out_f()));
  db.step1_2(edges({{1, 2}, {2, 3}}));
  print("t1", collect(db.t1_out_f()));
  db.seed1_1(nodes({7}));
  print("t1", collect(db.t1_out_f()));
  db.step1_2(edges({{3, 4}, {4, 5}, {6, 3}, {3, 1}, {7, 8}}));
  print("t1", collect(db.t1_out_f()));
  db.m1_1(nodes({5}));
  print("t1", collect(db.t1_out_f()));
  db.step1_2(edges({{5, 6}}));
  const auto f1 = collect(db.t1_out_f());
  print("t1 final", f1);

  // Family 2: seed first -- the recursive clause is live for every arrival.
  db.seed2_1(nodes({7}));
  print("t2", collect(db.t2_out_f()));
  db.m2_1(nodes({1}));
  print("t2", collect(db.t2_out_f()));
  db.step2_2(edges({{1, 2}, {2, 3}}));
  print("t2", collect(db.t2_out_f()));
  db.step2_2(edges({{3, 4}, {4, 5}, {6, 3}, {3, 1}, {7, 8}}));
  print("t2", collect(db.t2_out_f()));
  db.m2_1(nodes({5}));
  print("t2", collect(db.t2_out_f()));
  db.step2_2(edges({{5, 6}}));
  const auto f2 = collect(db.t2_out_f());
  print("t2 final", f2);
  ...
  db.m3_1(nodes({1}));
  print("t3", collect(db.t3_out_f()));
  db.step3_2(edges({{1, 2}, {2, 3}}));
  print("t3", collect(db.t3_out_f()));
  db.step3_2(edges({{3, 4}, {4, 5}, {6, 3}, {3, 1}, {7, 8}}));
  print("t3", collect(db.t3_out_f()));
  db.m3_1(nodes({5}));
  print("t3", collect(db.t3_out_f()));
  db.step3_2(edges({{5, 6}}));
  print("t3", collect(db.t3_out_f()));
  db.seed3_1(nodes({7}));
  const auto f3 = collect(db.t3_out_f());
  print("t3 final", f3);
