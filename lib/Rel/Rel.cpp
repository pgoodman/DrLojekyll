...
// walk. Symmetric with Build.cpp so the §7d role/walk cross-check never
// diverges. The flip's SET derivation uses this as the sole cut authority
// (SD-3). Defined at hyde scope (matching the header decl) so both Build.cpp
// and the anon-namespace AnyCutSuccessorDR call it.
