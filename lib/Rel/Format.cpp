...
static void EmitDRFlow(OutputStream &os, const DRFlowGraph &flow) {
  ...
  for (unsigned pi = 0u; pi < flow.pinned_order.size(); ++pi) {
    ...
    switch (op.kind) {
      ...
      // R-E42 (design §D7 / RH-7): the table-LESS monotone receive's ingest
      // LOOP. MARKER-SHAPE (header + `args:` line ONLY; NO reads/effects/spine
      // sublines — effect-free like the eager markers, §D3, and it sorts INTO
      // the subline-free lead-0 band). NEVER the kIngestFold case verbatim:
      // that prints `tid(op.ingest_table)` UNCONDITIONALLY and `tid()` has no
      // null guard, so it would CRASH on this table-less op — there is NO
      // `table=` token here. `message=` reuses the kIngestFold spelling (RH-8:
      // no new token, no E-71 lane; it names the drained receive's add-vec).
      ...
      // header renders `i#` only; full `store=I#` on args. s35 adaptation: the
      // `args:`-line base-table tokens carry the ` resource=sr#K` suffix (tidr —
      // the ONE goldened coverage subset); the descriptor line stays tid.
      case DROpKind::kSubgraphInstantiate: {
        ...
        emit_effects(op);
        emit_spine(op);
        os << "    args: demand=" << tidr(op.demand_table)
           << " pub=" << tidr(op.table_op_table)
           << " input=" << tidr(op.input_table) << " store=I#"
           << op.instance_store_id << "\n";
        break;
      }
      ...
      case DROpKind::kInstanceDeath: {  // live since D3.a.1 (differential demand)
        ...
        emit_effects(op);
        os << "    args: demand=" << tidr(op.demand_table)
           << " pub=" << tidr(op.table_op_table) << " store=I#"
           << op.instance_store_id << "\n";
        break;
      }
      ...
      case DROpKind::kInstanceSeal: {
        ...
        emit_effects(op);
        os << "    args: pub=" << tidr(op.table_op_table) << " store=I#"
           << op.instance_store_id << "\n";
        break;
      }
      ...
      // R-JOIN (rjoin-design §4): the pivot-JOIN and @product dispatch
      // markers. Byte-identical to the kEagerForward shape — NO extra token
      // (the pivot count re-derives from `eager_view` if ever needed — M2';
      // a discriminating pivots=N token was DECLINED unwitnessed). DEDICATED
      // cases per M7.
