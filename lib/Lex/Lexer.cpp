...
bool Lexer::TryGetNextToken(const StringPool &string_pool, Token *tok_out) {
  ...
  switch (ch) {
    ...
    case '@': {
      ...
      } else if (impl->data == "@key") {
        auto &basic = ret.As<lex::BasicToken>();
        basic.Store<Lexeme>(Lexeme::kPragmaKey);
        basic.Store<lex::SpellingWidth>(impl->data.size());
      ...
      } else if (impl->data == "@never") {
        auto &basic = ret.As<lex::BasicToken>();
        basic.Store<Lexeme>(Lexeme::kPragmaPerfNever);
        basic.Store<lex::SpellingWidth>(impl->data.size());

      } else if (impl->data == "@first") {
        auto &basic = ret.As<lex::BasicToken>();
        basic.Store<Lexeme>(Lexeme::kPragmaCodeGenFirst);
        basic.Store<lex::SpellingWidth>(impl->data.size());

      } else if (impl->data == "@barrier") {
        auto &basic = ret.As<lex::BasicToken>();
        basic.Store<Lexeme>(Lexeme::kPragmaPerfBarrier);
        basic.Store<lex::SpellingWidth>(impl->data.size());

      } else if (impl->data == "@nullable") {
        auto &basic = ret.As<lex::BasicToken>();
